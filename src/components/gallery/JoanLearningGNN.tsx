      ...
      >
        ...
        {ORBITS.map((orbit, i) => (
          <ellipse
            ...
            ry={orbit.ry}
            fill="none"
            stroke={['rgba(244,208,63,0.22)', 'rgba(52,152,219,0.16)', 'rgba(155,89,182,0.13)'][i]}
        ...
        {KNOWLEDGE_NODES.map((node) => {
          ...
          const size = [20, 17, 15][node.orbitIndex];
            ...
            >
              ...
              {hoveredNode === node.id && (
                <g transform={`translate(0, ${-size - 18})`}>
                  <rect x={-55} y="-16" width={110} height="22" rx="4"
                    fill="rgba(20,20,30,0.92)" stroke={cfg.fill} strokeWidth="1"/>
                  <text textAnchor="middle" y="0" fill="white" fontSize="10"
                    fontFamily="'Inter', sans-serif">
                    {node.labelCn ? `${node.labelCn} (${node.label})` : node.label}
                  </text>
                  <polygon points="-5,6 5,6 0,12" fill="rgba(20,20,30,0.92)"/>
                </g>
              )}
            </g>
          );
